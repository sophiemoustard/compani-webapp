    ...
    <div class="q-mb-xl">
      ...
      <q-card>
        ...
        <q-card-actions align="right">
          <ni-button color="primary" icon="add" label="Ajouter un(e) testeur/testeuse" :disable="loading"
...
export default {
  ...
  methods: {
    async nextStepTesterCreationModal () {
      try {
        ...
        if (userInfo.exists) {
          ...
          NotifyPositive('Testeur/testeuse ajouté(e) avec succès.');
        ...
        NotifyNegative('Erreur lors de l\'ajout du testeur/de la testeuse.');
      } finally {
    ...
    validateTesterDeletion (testerId) {
      this.$q.dialog({
        title: 'Confirmation',
        message: 'Êtes-vous sûr(e) de vouloir retirer le testeur/la testeuse du programme ?',
        ok: true,
        cancel: 'Annuler',
    ...
    async deleteTester (testerId) {
      try {
        ...
        this.$emit('refresh');
        NotifyPositive('Testeur/testeuse supprimé(e).');
      } catch (e) {
        console.error(e);
        NotifyNegative('Erreur lors de la suppression du testeur/de la testeuse.');
      }
    },
  },
};
